...
import (
	...
	"github.com/Cxiyuan/NTA/services/intel-service/internal/api"
...
func main() {
	...
	dsn := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable search_path=intel_schema",
		*dbHost, *dbPort, *dbUser, *dbPass, *dbName)

	db, err := gorm.Open(postgres.Open(dsn), &gorm.Config{})

	// Create schema if not exists
	db.Exec("CREATE SCHEMA IF NOT EXISTS intel_schema")
	if err != nil {
		logger.Fatalf("Failed to connect to database: %v", err)
	}

	repo := repository.NewIntelRepository(db)
	svc := service.NewIntelService(repo, logger)
	handler := api.NewIntelHandler(svc, logger)
	...
	logger.Infof("Starting intel-service on %s", addr)
