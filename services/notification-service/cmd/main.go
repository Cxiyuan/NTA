...
import (
	...
	"github.com/Cxiyuan/NTA/services/notification-service/internal/api"
...
var (
	port     = flag.Int("port", 8086, "Server port")
...
func main() {
	...
	dsn := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable search_path=notification_schema",
		*dbHost, *dbPort, *dbUser, *dbPass, *dbName)

	db, err := gorm.Open(postgres.Open(dsn), &gorm.Config{})

	// Create schema if not exists
	db.Exec("CREATE SCHEMA IF NOT EXISTS notification_schema")
	if err != nil {
	...
	handler := api.NewNotificationHandler(svc, logger)
	...
	logger.Infof("Starting notification-service on %s", addr)
